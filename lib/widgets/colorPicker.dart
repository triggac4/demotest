...
class ColorPicker extends LeafRenderObjectWidget {
  const ColorPicker({ required this.vsync, required this.onChanged});
...
class RenderColorPicker extends RenderBox {
  ...
  void _onStartDrag(DragStartDetails details) {
    slideController.value =
        _resolveHorizontalOffsetFromLocalPosition(details.localPosition);
    markNeedsPaint();
  }
  ...
  Color get selectedColor =>
      _resolveColorFromHue(selectedPercent * maxColorHue);
  ...
  bool hitTestSelf(Offset position) =>
      controlHandleBounds.contains(position) ||
          trackHandleBounds.contains(position);
  ...
  void paint(PaintingContext context, Offset offset) {
    ...
    trackHandleBounds =
        RRect.fromRectAndRadius(trackBounds, Radius.circular(trackHeight / 2));
    ...
    canvas.drawRRect(
      ...
      Paint()
        ..shader =
        LinearGradient(colors: colorsSpectrum).createShader(trackBounds),
    );

    // Compute control handle & preview
    final handleBounds = Rect.fromCircle(
      center: Offset((selectedPercent * trackBounds.width) + trackBounds.left,
          trackBounds.center.dy),
      radius: _controlHandleHeight / 2,
    );
    controlHandleBounds = RRect.fromRectAndRadius(
        handleBounds, Radius.circular(_controlHandleHeight / 4));
    ...
    final previewCenter =
    previewOffset.translate(0, -previewRadius - triangleLength);
    ...
    previewPath
      ..addPath(
        Path()
          ..moveTo(handleBounds.center.dx - iconSpacing,
              handleBounds.center.dy - iconSize)
    ...
    canvas.drawPath(
      ...
      Paint()
        ..color = shadowColor
        ..maskFilter =
        ui.MaskFilter.blur(BlurStyle.normal, previewRadius / 1.5),
    );
    // Draw control handle
    ...
    canvas.drawRRect(
      RRect.fromRectAndRadius(
        Rect.fromCircle(
            center: previewCenter, radius: previewRadius - borderPadding),
  ...
  Size _computeSize(BoxConstraints constraints) {
    return constraints.constrain(Size(
        _initialPreferredSize.width, _controlHandleHeight + _previewHeight));
  }

  Size get _initialPreferredSize {
    final maxSize = constraints.biggest;
    return Size(maxSize.width.clamp(100.0, 800.0).toDouble(),
        maxSize.height.clamp(0.0, 300.0).toDouble());
  }
